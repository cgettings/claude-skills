...
**Ask "was it in context" first, because it is the only one of the three with a mechanical answer.** If the environment logs instruction-file loads, that log answers it directly; otherwise the evidence is this session's own transcript — the skill invocation or file read that would have pulled the tier in. An always-loaded file is the case the transcript cannot answer: it is injected rather than read, so it leaves no call to find, and a grep returns a null with no power. That tier needs the load log or nothing. Either way the answer is a **null**, so prove the probe can fire before reporting one: run the same check against a rule you know was loaded this session.

**Record the diagnosis where it can be counted, not beside the rule.** One miss is an anecdote. Six that all read *never in context, on a skill* is a verdict on the routing rule rather than on six rules, and per-rule notes cannot be counted — nor do they have anywhere to live for a rule whose evidence was never split out. One line per miss in one file: date, rule, the tier it lives in, why it missed, what was repaired. Three lines against the same rule is a `refile-rules` trigger on its own. Put that file wherever this environment keeps durable notes that are not themselves rules — and search for it before creating it, under any name and from the store's root rather than the subdirectory you expect it in, since two series cannot be counted together and the second one silently ends the first. If it genuinely does not exist, give it its own file rather than tucking the series into a document that exists for something else.

**Then write its path into the always-loaded tier**, as one clause beside whatever already points at this skill. The path is a rare thing that earns that space on this skill's own test: you need it at the moment you are about to record a miss, and at that moment you would not know to go looking for it — which is precisely how a second log gets created beside the first. The skill cannot carry the path itself, because the path is this environment's and the skill is not. A pointer costs one line; a series split across two files cannot be counted at all.
